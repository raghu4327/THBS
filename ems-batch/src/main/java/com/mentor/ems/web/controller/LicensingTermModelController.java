...
@RestController
@RequestMapping(value = "/autoRenewalBatch")
public class LicensingTermModelController {

	private static final Logger LOGGER = LoggerFactory.getLogger(LicensingTermModelController.class);

	@Autowired
	JobLauncher jobLauncher;

	@Autowired
	@Qualifier("TermLicenseJob")
	Job job;
	...
	public ResponseEntity<JobStatusDTO> submit() {
		...
		parameterMap.put(EMSCommonConstants.PRODUCT_ENT_DETAIL_SQL_QUERY_RAW, sqlQueryRawParam);
		...
		LOGGER.info(" Exit from LicensingTermModelController -submit method - Batch processing:...");
		return ResponseEntity.status(HttpStatus.ACCEPTED).body(jobStatusDTO);
	}

	private String getViewProductRawSQLQuery() {
		String licenseRenewalConfigDay = EMSConfigLoader.getConfigValue(EMSCommonConstants.APP_CONFIG,
				EMSCommonConstants.TERM_RENEW_DAY);
		StringBuilder sqlQueryRaw = new StringBuilder(getSelectClause()).append(" FROM ").append(getSchemaName())
				...
				//.append(EMSCommonConstants.JOIN_QUERY)
				.append(getWHEREClause()).append("+").append(licenseRenewalConfigDay).append(")");
		LOGGER.debug("getViewProductRawSQLQuery()_ sqlQueryRaw=" + sqlQueryRaw);
	...
	private String getWHEREClause() {
		return " WHERE " + EMSCommonConstants.PRODUCT_ENT_DTL_WHERE_CLAUSE;
	}
