...
/**
 * This class will launch the ongoing Solr Consulting Entitlements
 * 
 * @author avijeetk
 *
 */
@Service
public class SolrConsultingEntitlementOngoingBatchLauncher {

    private static final Logger LOGGER = LoggerFactory.getLogger(SolrConsultingEntitlementOngoingBatchLauncher.class);

    @Autowired
    JobLauncher jobLauncher;

    @Autowired
    @Qualifier("SOLRConsultingEntitlementOngoing")
    Job job;   

    public JobStatusDTO submitBatchRequest() {
        JobStatusDTO jobStatusDTO = null;

        LOGGER.info("submitBatchRequest()... SOLRConsultingEntitlementOngoing Sync....");
        ...
        LOGGER.info("Exit from SOLRConsultingEntitlementOngoing Sync");
        return jobStatusDTO;
    }
}
