...
import com.mentor.ems.batch.solr.entitlement.processor.SolrEntitlementProcessor;
...
/**
 * Component - Writer class for - Solr Entitlement - Delta processing
 */
@Component
public class SolrEntitlementOngoingWriter implements  ItemWriter<EmsJob> {

    private static final Logger LOGGER = LoggerFactory.getLogger(SolrEntitlementOngoingWriter.class);

    @Autowired
    EntitlementIndexService entitlementIndexService;

    @Autowired
    SolrEntitlementProcessor solrEntitlementProcessor;
    ...
    public void write(List<? extends EmsJob> items) throws Exception {
        for ( EmsJob emsJob : items ) {
            try {
                ...
                List<EntitlementSearchDTO> entitlementSearchDTOList = entitlementViewDAO.getEntitlementsFromJobId(emsJob.getJobKey());
                LOGGER.info("EmsJob.job_key=" + emsJob.getJobKey() + " Read complete ");

                for (EntitlementSearchDTO entitlementSearchDTO : entitlementSearchDTOList) {
                    solrEntitlementProcessor.process(entitlementSearchDTO);
                }
