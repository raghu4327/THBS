...
/**
 * Component - Writer class for - Solr Consulting Entitlement - Delta processing
 */
@Component
public class SolrConsultingEntitlementOngoingWriter implements  ItemWriter<EmsJob> {

    private static final Logger LOGGER = LoggerFactory.getLogger(SolrConsultingEntitlementOngoingWriter.class);
    ...
    public void write(List<? extends EmsJob> items) throws Exception {
        for ( EmsJob emsJob : items ) {
            try {
                ...
                List<EntitlementSearchDTO> entitlementSearchDTOList = entitlementViewDAO.getConsultingEntitlementsFromJobId(emsJob.getJobKey());
