 ...
 *
 */
public class SolrCustomerOngoingChunkListener implements JobExecutionListener {
	
	@Autowired
	private EMSJobDAO emsJobDAO;
	

	private static final Logger LOGGER = LoggerFactory.getLogger(SolrCustomerOngoingChunkListener.class);
	...
	public void afterJob(JobExecution jobExecution) {
		...
		int count;
		try {
			count = emsJobDAO.updateFailedJobStatus(EMSCommonConstants.SOLR_CUSTOMER_ONGOING_JOB);
