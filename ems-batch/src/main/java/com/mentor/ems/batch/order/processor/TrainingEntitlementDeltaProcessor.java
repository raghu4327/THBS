...
@Component
public class TrainingEntitlementDeltaProcessor implements ItemProcessor<OrderDetail, JobPayloadDTO> {
	private static final Logger LOGGER = LoggerFactory.getLogger(TrainingEntitlementDeltaProcessor.class);
	...
	public JobPayloadDTO process(OrderDetail orderDetail) throws Exception {
		LOGGER.info("Reading EmsJob from EMS_JOB table " + orderDetail);
	...
	private EntitlementJobRequestDTO getEntitlementJobRequestDTOJobParams(OrderDetail orderDetail) {
		return entitlementUtil.getEntitlementDetlaRequest(orderDetail, EntitlementType.TRAINING);
	}
