package com.mentor.ems.batch.licensing.term.writer;
...
@Component
@Transactional
public class AutoRenewalWriter implements ItemWriter<ProductEntitlementDetailDTO> {
	private static final Logger LOGGER = LoggerFactory.getLogger(AutoRenewalWriter.class);
	...
	public void write(List<? extends ProductEntitlementDetailDTO> items) throws Exception {
		...
		LOGGER.info("Enter into AutoRenewalTermWriter - write method");
			try {
				
				batchMaster = batchMasterDao.createBatchMaster(EMSCommonConstants.BATCH_TYPE_LC_AUTO_TERM,
                        EMSCommonConstants.BATCH_SOURCE_INPUT_TYPE_JSON, EMSCommonConstants.EMS);
				
				//batchMaster = batchMasterDao.createBatchMaster(EMSCommonConstants.BATCH_TYPE_LICENSEREQ_AUTO_TERM);
				...
				LOGGER.error("Exception occured in AutorenewalTermWriter ", e);
			}
	 ...
	 */
	private Map<Long,List<ProductEntitlementDetailDTO>> groupingBasedOnsites(List<? extends ProductEntitlementDetailDTO> items) {
		...
		/*for (ProductEntitlementDetailDTO productEntitlementDetailDTO : items) {
			Long siteNumber = new Long(productEntitlementDetailDTO.getSiteNbr());
			if (siteMap.containsKey(siteNumber)) {
				list = siteMap.get(siteNumber);
				list.add(productEntitlementDetailDTO);
				siteMap.put(siteNumber, list);
			} else {
				list = new ArrayList<ProductEntitlementDetailDTO>();
				list.add(productEntitlementDetailDTO);
				siteMap.put(siteNumber, list);
			}
		}*/
	...
	public void processLicenseBySite(Map<Long, List<ProductEntitlementDetailDTO>> siteAndIBMap,BatchMaster batchMaster,
		...
		for (Entry<Long, List<ProductEntitlementDetailDTO>> productEntitlementSite : siteAndIBMap.entrySet())
			{
				...
				createEvent = eventDao.createEvent("LA"+"|"+siteNumber+"|"+notificationNbr, "TERM",
				...
				if (attrValue.equalsIgnoreCase("YES")) {
					...
					automaticCode = codeDao.getCodeByTypeAndDescr(EMSCommonConstants.AUTHCODE_DELIVERY_STATUS_CODE, "Automatic");
					emsWorkflowProcess.setAuthcodeStatus(automaticCode);
					pauseCode = codeDao.getCodeByTypeAndDescr(EMSCommonConstants.ELD_AUTORENEWAL_STATUS_CODE,
							"Paused");
					emsWorkflowProcess.setAuthcodeStatus(automaticCode);
				...
				Codes autorenewal = codeDao.getCodeByTypeAndDescr(EMSCommonConstants.ELD_AUTO_RENEWAL,
						"AUTO RENEWAL");
				pendingCode = codeDao.getCodeByTypeAndDescr(EMSCommonConstants.ELD_AUTORENEWAL_STATUS_CODE,
						"Pending");
				emsWorkflowProcess.setEldNotificationStatus(pendingCode);
				emsWorkflowProcess.setProcessNameCdKey(autorenewal);
				Codes newCode = codeDao.getCodeByTypeAndDescr(EMSCommonConstants.ELD_AUTORENEWAL_STATUS_CODE,
							"NEW");
				...
				// payload process commented for testing purpose
		...
		LOGGER.info("Enter intoAutoRenewalWriter:processLicenseBySite Exception");
		batchMasterDao.updateBatchStatus(batchMaster, EMSCommonConstants.FAILED);
		...
		throw new Exception(
				"Exception occured in processLicenseBySite while proccesing LicenseRequisitionResponse",
				e);
	} 
	...
	public void onWriteError(Exception e,List<? extends InstalledItemDTO> items) {
