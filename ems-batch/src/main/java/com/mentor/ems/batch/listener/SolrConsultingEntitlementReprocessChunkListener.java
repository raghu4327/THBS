 ...
 *
 */
public class SolrConsultingEntitlementReprocessChunkListener implements JobExecutionListener {
	
	@Autowired
	private EMSJobDAO emsJobDAO;
	

	private static final Logger LOGGER = LoggerFactory.getLogger(SolrConsultingEntitlementReprocessChunkListener.class);
	...
	public void afterJob(JobExecution jobExecution) {
		...
		int count;
		try {
			count = emsJobDAO.updateFailedJobStatus(EMSCommonConstants.SOLR_CONSULTING_ENTITLEMENT_REPROCESSING_TYPE_JOB);
